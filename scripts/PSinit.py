...
def extract_video_frame(file_name): # pyright: ignore[reportRedeclaration]
...
mask_img_inv = cv2.bitwise_not(mask_img) # pyright: ignore[reportArgumentType, reportCallIssue]

# Combine the probability map with the inverted mask
# pyrefly: ignore [no-matching-overload]
prob_map_combined = cv2.bitwise_and(prob_map, prob_map, mask=mask_img_inv) # pyright: ignore[reportArgumentType, reportCallIssue]
...
for contour in contours:
    ...
    if area < region_thresholds['min_area'] and solidity < region_thresholds['min_solidity']: # pyright: ignore[reportPossiblyUnboundVariable]
        continue
    ...
    if not (region_thresholds['min_width'] <= w <= region_thresholds['max_width'] and # pyright: ignore[reportPossiblyUnboundVariable]
            region_thresholds['min_height'] <= h <= region_thresholds['max_height']): # pyright: ignore[reportPossiblyUnboundVariable]
        continue  # Skip contour if it doesn't meet size constraints

    aspect_ratio = max(w, h) / min(w, h) if min(w, h) > 0 else max(w, h)

    if aspect_ratio > region_thresholds['max_aspect_ratio']: # pyright: ignore[reportPossiblyUnboundVariable]
        continue
    ...
    if solidity < region_thresholds['min_solidity']: # pyright: ignore[reportPossiblyUnboundVariable]
        continue
...
# pyrefly: ignore [no-matching-overload]
overlay_image = cv2.addWeighted(segmented_im, 0.7, labeled_image_bgr, 0.3, 0) # pyright: ignore[reportArgumentType, reportCallIssue]
...
# pyrefly: ignore [no-matching-overload]
overlay_image = cv2.addWeighted(hsv_mask, 0.7, result_image_bgr, 0.3, 0) # pyright: ignore[reportArgumentType, reportCallIssue]
...
for root, dirs, files in os.walk(input_folder):
    for i, video_file in enumerate(files):
        ...
        while current_frame < total_frames:
            ...
            if ret: # pyright: ignore[reportPossiblyUnboundVariable]
                frame_path = os.path.join(output_folder, f'video_{i}_frame{current_frame}.png')
                cv2.imwrite(frame_path, frame) # pyright: ignore[reportPossiblyUnboundVariable]
...
# pyrefly: ignore [no-matching-overload]
file_names.sort(key=extract_video_frame) # pyright: ignore[reportArgumentType, reportCallIssue]
...
for image_file in file_names:
    ...
    # pyrefly: ignore [no-matching-overload]
    image_hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) # pyright: ignore[reportArgumentType, reportCallIssue]
...
if image_count > 0:
    ...
    probability_map = accumulator / image_count # pyright: ignore[reportOptionalOperand]
    ...
    # pyrefly: ignore [no-matching-overload]
    result = cv2.imwrite(output_path, probability_map_image) # pyright: ignore[reportArgumentType, reportCallIssue]
    if result:
...
coords = []
def click_event(event, x, y, flags, param): # pyright: ignore[reportRedeclaration]
    if event == cv2.EVENT_LBUTTONDOWN:
        coords.append((x, y))
        # pyrefly: ignore [no-matching-overload]
        cv2.circle(img, (x, y), 3, (0, 255, 0), -1) # pyright: ignore[reportArgumentType, reportCallIssue]
        # pyrefly: ignore [no-matching-overload]
        cv2.imshow('image', img) # pyright: ignore[reportArgumentType, reportCallIssue]
...
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [upper_level_l], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [upper_level_m], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [upper_level_r], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportCallIssue, reportArgumentType]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [close_perp], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportCallIssue, reportArgumentType]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [far_side], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [close_side], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [far_perp], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]
# pyrefly: ignore [no-matching-overload]
cv2.polylines(parking_spaces, [small_park], isClosed=True, color=(0, 255, 0), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]
for region in ignore_regions:
    # pyrefly: ignore [no-matching-overload]
    cv2.polylines(parking_spaces, [region], isClosed=True, color=(0, 0, 255), thickness=2) # pyright: ignore[reportArgumentType, reportCallIssue]

# Display the image with regions
# pyrefly: ignore [no-matching-overload]
cv2.imshow('Parking Spaces with Regions', parking_spaces) # pyright: ignore[reportArgumentType, reportCallIssue]
cv2.waitKey(0)
cv2.destroyAllWindows()
...
def click_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        # pyrefly: ignore [no-matching-overload]
        cv2.circle(image, (x, y), 5, (0, 255, 0), -1) # pyright: ignore[reportArgumentType, reportCallIssue]
        points.append((x, y))

        if len(points) >= 2:
            # pyrefly: ignore [no-matching-overload]
            cv2.line(image, points[-2], points[-1], (255, 0, 0), 2) # pyright: ignore[reportArgumentType, reportCallIssue]
            x1, y1 = points[-2]
            x2, y2 = points[-1]
            width = abs(x2 - x1)
            height = abs(y2 - y1)
        ...
        # pyrefly: ignore [no-matching-overload]
        cv2.imshow('image', image) # pyright: ignore[reportArgumentType, reportCallIssue]
